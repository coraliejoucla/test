...
fileList = uipickfiles('FilterSpec', 'I:\own-name', 'Type', {'*.vhdr', 'header-files'}); % pick .vhdr file %%%%% modify
...
smileOwnNameMarker = 'R 24';
roughOwnNameMarker = 'R 28';
...
new_riseOwnNameMarker = 'riseOwnName';
new_fallOwnNameMarker = 'fallOwnName';

finalMarkerList = {new_frequentMarker, new_neutralOwnNameMarker, new_riseOwnNameMarker, new_fallOwnNameMarker};

topoFlag = 0; % 0 for pvalue topography, 1 for significant channels
alphaThreshold = 0.05;

nbChannels = 32; % nb of electrodes recorded
chanToStat = {'Fz'};
...
for iter = 1:length(fileList) % i.e. for each subject
    ...
    for ii = 1:length(originalEEG.event)
        ...
        elseif isequal(originalEEG.event(ii).type, 'R 12')
            countR12 = countR12+1;
        elseif isequal(originalEEG.event(ii).type, 'R 24')
            countR24 = countR24+1;
        elseif isequal(originalEEG.event(ii).type, 'R 28')
            countR28 = countR28+1;
        end
    end
