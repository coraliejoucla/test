function [clusters, p_values, t_sums, permutation_distribution ] = permutest( trial_group_1, trial_group_2, flagDimension, dependent_samples, ...
...
if nargin < 8 || isempty(num_clusters)
    num_clusters = inf;
end
if nargin < 7 || isempty(two_sided)
    two_sided = false;
end
if nargin < 6 || isempty(num_permutations)
    num_permutations = 10^4;
end
if nargin < 5 || isempty(p_threshold)
    p_threshold = 0.05;
end
if nargin < 4 || isempty(dependent_samples)
